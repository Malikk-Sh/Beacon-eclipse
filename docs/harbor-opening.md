...
The orientation and trusted-touch checks run in a fresh mobile context, separately from the two-boot settings journey, with the original 240-second timeout unchanged. Step timings are written to CI logs. Review captures use CSS resolution and the title capture is not repeated on reload. This keeps software-WebGL capture/boot cost from consuming the orientation budget while preserving the assertions from PR #43.
