...
export const HARBOR_DISCOVERIES = [
  { id: 'coastal-survey', title: 'БЕРЕГОВОЙ ПОСТ — ПЕЛЕНГ СИГНАЛА', x: -19, z: 32,
    speaker: 'МАРА', text: 'На карте три линии: маяк, склад, школа. Под ними: «Замыкать только на мостовом ретрансляторе. В море не передавать». Одна линия нарисована с обратной стороны бумаги.' },
  { id: 'watch-log', title: 'ЖУРНАЛ СМОТРИТЕЛЯ', x: -2.55, z: 28,
    speaker: 'ЛЕВ', text: '«22:47. Восемь ударов. 22:47. Восемь ударов. 22:47. Не отвечать своим голосом». Каждая строка написана другими чернилами. Дата одна.' },
  { id: 'harbor-receiver', title: 'ПРИЧАЛ 02 — ПРОВЕРИТЬ ПРИЁМНИК', x: -50, z: 5.1,
    speaker: 'МАРА', text: 'Плёнка движется назад. На корпусе выцарапано: «НИКА — не человек. Позывной нижнего канала». Под крышкой ни одной батареи.' },
  { id: 'ferry-board', title: 'ПАРОМНЫЙ ТЕРМИНАЛ — ОСМОТРЕТЬ', x: 47, z: 3,
    speaker: 'ЛЕВ', text: 'Билет на последний паром. Время отправления — 22:48. Имя пассажира: Лев Арден. Я не покупал этот билет.' },
] as const;
...
export class HarborDistrict {
  ...
  private lighthouseInterior(): void {
    ...
    this.sign('ЖУРНАЛ СМЕНЫ', '22:47   /   ПОВТОР № 08', -3.05, 2.57, 28, 1.2, 0.42, Math.PI / 2);
  ...
  private ferryPier(): void {
    ...
    this.sign('ПОСЛЕДНИЙ ПАРОМ', '22:48  /  ОЖИДАНИЕ ПРИЛИВА', 48, 2.1, 1.43, 2.1, 0.9);
